...
DESTINO=spoaxap8
TMP=/tmp/BSCS_IIH$$.tmp
TMP1=/tmp/BSCS_IIH1$$.tmp
TMP2=/tmp/BSCS_IIH2$$.tmp
TMP3=/tmp/${SITE}IIH${DATA}.txt

cd ~prod/WORK/TMP
[ $? != 0 ] && exit 1

# Procura arquivos nao marcados com bit de execucao para other
find . ! -perm -1 -name "IIH*" -print |\
while read file
   do case "$file" in
         *.PRT) MSG=I-RATING-IIH-PROTOCOLO
                FILETYPE="PROTOCOL" ;;
         *.ERR) MSG=E-RATING-IIH-ERRO
                FILETYPE="ERROR" ;;
             *) MSG=W-RATING-IIH-OUTROS
                FILETYPE="UNKNOWN" ;;
      esac
        ...
        case $FILETYPE in
           PROTOCOL ) # Arquivo de protocol 
                      ...
                      awk -F ":" 'BEGIN { 
                       ...
                       datafile=`echo $DATAFILE | awk '{print (substr ($0, match ($0, "IIH.............PRT")))}'`
                       
                       (echo $datafile - COBERTURA DE HORARIO DO RATING - DAP
                        echo ==========================================================
                        echo
                        cat $TMPDAP)  | msg_api2 "I-RATING-IIH-DAP
                       (echo $datafile - COBERTURA DE HORARIO DO RATING - NORTEL 
                        echo =============================================================
                        echo
                        cat $TMPNORTEL) | msg_api2 "I-RATING-IIH-NORTEL
                        cat $TMPNORTEL
              ...
              ERROR ) # Arquivo de erros
                      # Copia mensagens com prefixo IIH: 
                      # Copia mensagens com prefixo Error
                      # Conta mensagens semelhantes 
                      cat -s $file | grep -e IIH: -e Error \
                      -e "already processed" |
