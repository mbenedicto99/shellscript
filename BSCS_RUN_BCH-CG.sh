...
MsgErro="${MsgErro}\n Sintaxe:\n   BSCS_RUN_BCH-CG.sh <CICLO> <Grupo de CG [A-O]> <Forca data Venc??>"
F_ValidaRc $RC "${MsgErro}"

#  Carrega variaveis com nome dos arquivos de LIBERACAO de CICLO.
FILE_AUTH_BCH=${DIR_AUTH}/"BCH-"${BILLCYCLE}.flg
FILE_AUTH_BGH=${DIR_AUTH}/"BGH-"${BILLCYCLE}.flg

#  Verifica se os arquivos de liberacao do CICLO (BCH e BGH) estao disponiveis.
[  -f ${FILE_AUTH_BCH} ] && [ -f ${FILE_AUTH_BGH} ]
RC=$?

MsgErro="Nao existe(m) Arquivo(s) de liberacao de CICLO!!!!!!! \n `ls -ltr ${FILE_AUTH_BCH}` \n `ls -ltr ${FILE_AUTH_BGH}`"
F_ValidaRc $RC "${MsgErro}"

#  Verifica se o SYNONYM DOCUMENT_ALL esta associado a DOC_ALL_## do ciclo em processamento.
...
#  no formato DDMMAA.
DI=`sed -n '2p' $FILE_AUTH_BGH`
DF=`sed -n '3p' $FILE_AUTH_BGH`
DV=`sed -n '4p' $FILE_AUTH_BGH`
 
#  Extracao de data Vencimento para as Faturas do CICLO no aq. de lib. do BCH
DC=`sed -n '2p' $FILE_AUTH_BCH`
...
BchDataVenc="`echo ${DV} | awk '{ print substr($1,5,2) substr($1,3,2) substr($1,1,2)'}`"
...
############# MARCOS DE BENEDICTO 16/03/2004  #####################
     ...
     echo "*************** Problema no UPDATE GEOCODE!!!"
     echo "*************** Problema no UPDATE GEOCODE!!!"
     echo " STATUS CODE DO SQLPLUS:  $RC "
     exit $RC
  fi
...
then
   ...
   then
      echo "
          ...
          cp /tmp/`basename $FILE_AUTH_BCH` $FILE_AUTH_BCH\n"
      ...
      then 
         echo "
               ...
               \t Entrar em contato com o responsavel pelo scheduler.\n
               \t Processo abortado.\n
