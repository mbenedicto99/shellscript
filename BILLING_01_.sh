#!/bin/ksh
#  Script      : BILLING_01_02.sh
#  Objetivo    : PROCESSAMENTO ANALISE DE LOG
#  Descricao   : 
#  Pre-Requis  : 
...
cd $DIR
RC=$?
echo "Validando a existencia do Diretorio!!! Caso o mesmo nao exista (erro no comando cd) o JOB emite ABEND!!!"
[ $RC != 0 ] && exit 1

if [ -z "${DIR}" ]
then
   echo "Erro na especificacao da variavel DIR: ${DIR}. Verificar!!!!! "
   exit 99
fi
...
while read file
   do case "basename $file" in
       *PBCH*.log) MSG=I-BILLING-BCH-002
