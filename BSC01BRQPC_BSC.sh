#!/bin/ksh
	...
	EMAIL_REL="relat_tels_disponiveis@nextel.com.br,rafael.toniete@nextel.com.br"
	EMAIL_ERR="relat_tels_disponiveis@nextel.com.br"
	DESC="Relatorio de numeros de telefones."
   ...
   . /amb/eventbin/SQL_RUN.PROC3 "${TWO_TASK}" "${USRPASS}" "${SQL}" "${EMAIL_ERR}" "${DESC}" 0 BSCS ${SPOOL}

	#${ORACLE_HOME}/bin/sqlplus / @${SQL} >${SPOOL} 2>&1

	/amb/operator/bin/attach_mail ${EMAIL_REL} ${SPOOL} "${DESC}"
