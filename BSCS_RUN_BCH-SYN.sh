...
###----------------------------------------------------------------------------
### Alterado no processo de desmembramento da DOCUMENT_ALL (2003/10/27).
###----------------------------------------------------------------------------
...
printf "%s\t%s\t%s\t%s\n" "BCH_CG_${BILLCYCLE}" "Inicio do processamento." "${LOC_TIME1}" "${LOC_TIME2}" >>${LOG_TIME}
...
FLG=33
	...
	set -x
	echo ${ARQ_BCHLOG}
	COUNT=`grep -i -c "FATAL ERROR" ${ARQ_BCHLOG}`
...
printf "%s\t%s\t%s\t%s\n" "BCH_CG_${BILLCYCLE}" "Termino do processamento." "${LOC_TIME1}" "${LOC_TIME2}" >>${LOG_TIME}
