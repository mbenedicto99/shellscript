...
CHK_CICLO=$1
	...
	[ -f "${ENV_DIR_BASE_RTX}/prod/WORK/TMP/BILL-CG${CHK_CICLO}.flg" ] && exit 0

#DATE_08="`date +%Y%m%d`"
#[ "${CHK_CICLO}" -eq 08 -a "${DATE_08}" -eq 20061228 ] && exit 0
#[ "${CHK_CICLO}" -eq 08 -a "${DATE_08}" -eq 20061229 ] && exit 0
#[ "${CHK_CICLO}" -eq 08 -a "${DATE_08}" -eq 20061230 ] && exit 0
#[ "${CHK_CICLO}" -eq 08 -a "${DATE_08}" -eq 20061231 ] && exit 0
...
case $CICLO
in
  0[1-9]|1[0-7]) echo "...... Ciclo Definido corretamente!! Ciclo: $CICLO"
                 ;;
              *) echo " Valor de ciclo invalido !!!!! Ciclo: $CICLO"
                 echo " Valores validos para ciclo: 01-17 "
                 exit 99
                 ;;
esac
...
echo "Final da contagem dos RTX : `date`"
      ...
      su - prod -c "/amb/operator/bin/run_rlh ${CICLO}"
